document.addEventListener('DOMContentLoaded', function() {
    ...
    // 13. منطق التحقق من تسجيل الدخول وحفظ التوكن وتوجيه الصلاحيات
    if (loginForm) {
        loginForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (result.ok && result.status === 200) {
                    ...
                    if (emailInput === "admin@ertiwa.com") { 
    ...
    // 13. [محدث ومربوط بالـ API] منطق التحقق من تسجيل الدخول وحفظ التوكن
    if (loginForm) {
        loginForm.addEventListener('submit', async function(e) {
            e.preventDefault();

            // 1. جلب القيم من الخانات داخل صفحة تسجيل الدخول
            const emailInput = document.getElementById('email')?.value; 
            const passwordInput = document.getElementById('password')?.value; 
            ...
            // تلميح أمان: إظهار مؤشر تحميل بسيط للمستخدم أثناء انتظار السيرفر المجاني ليفتح
            const submitBtn = loginForm.querySelector('button[type="submit"]');
            const originalText = submitBtn ? submitBtn.textContent : "تسجيل الدخول";
            if (submitBtn) submitBtn.textContent = "... جاري التحقق من البيانات";

            try {
                // 2. استدعاء دالة الـ API التي وضعناها في ملف api.js
                const result = await loginWithAPI(emailInput, passwordInput);

                // 3. الفحص البرمجي الهندسي للرد
                if (result.ok && result.status === 200) {
                    alert('تم تسجيل الدخول بنجاح!');
                    // حفظ التوكن الحقيقي القادم من السيرفر في ذاكرة المتصفح
                    localStorage.setItem('userToken', result.data.access_token);
                    // تحويل الآدمن فوراً إلى لوحة التحكم
                    window.location.href = 'dashboard.html'; 
                } else {
                    // إذا كان الباك إند مبرمجاً بشكل صحيح، سيصل هنا ويعطيكِ رسالة الخطأ
                    alert('فشل تسجيل الدخول: ' + (result.data.message || "البيانات غير مطابقة"));
                }
            } catch (error) {
                console.error("حدث خطأ غير متوقع أثناء الاتصال:", error);
            } finally {
                // إعادة نص الزر لوضعه الطبيعي بعد انتهاء المحاولة
                if (submitBtn) submitBtn.textContent = originalText;
            }
        });
    }
